...
namespace SodaCL.Core.Game
{
	internal class MinecraftLaunch
	{
		...
		{
			var mcVersionJson = System.IO.File.ReadAllText(Launcher.LauncherInfo.mcVersionsDir + "\\" + versionName + "\\" + versionName + ".json");
			Log(false, ModuleList.IO, LogInfo.Info, "SodaCL 已对核心 Json 进行解析，结果如下: ");
			Log(false, ModuleList.IO, LogInfo.Info);

			var indexVersion = "1.12";
			int indexVersionInt;
			var javaSelectedPath = new string[100];
			var launcherVersion = "0.0.4.5";
			var uuid = "4a24d080533d310e9f4fab2a59b541fe";
			var accessToken = "c7c2a2390685472da58d2838c6f6dfbc";
			var windowWidth = "756";
			...
			{
				Log(true, ModuleList.IO, LogInfo.Info, "SodaCL 无法启动游戏, 因为找不到合适的 Java 。");
			}
			else
			{
